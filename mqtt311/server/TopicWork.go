...
func (s *TopicWork) matchTopic(topic string) []string {
	...
	for i := 0; i < 3; i++ {
		s.dfs(tp, i, 1, tp[0], &ret)
	}
	return ret
}
func (s *TopicWork) dfs(tps []string, index, level int, tempTp string, ret *[]string) {
	if index == 0 {
