...
func (s *SUBSCRIBERouter) Handle(request *Request) {
	...
	p := proto.NewSUBACKProtocol(uint32(len(sp.TopicFilterList)),
		sp.PacketIdentifier, make([]byte, len(sp.TopicFilterList)))

	//p := &proto.SUBACKProtocol{
	//	Fixed: &proto.Fixed{
	//		HeaderFlag: proto.SUBACK,
	//		MsgLen:     uint32(2 + 1 + len(sp.TopicFilterList)), // 标识符 2 字节，属性1 返回code len(sp.TopicFilterList)
	//		Data:       nil,
	//	},
	//	// 和订阅协议相同
...
func (s *UNSUBSCRIBERouter) Handle(request *Request) {
	...
	p := proto.NewUNSUBACKProtocol(uint32(len(sp.TopicFilterList)),
		sp.PacketIdentifier, make([]byte, len(sp.TopicFilterList)))

	//p := &proto.UNSUBACKProtocol{
	//	Fixed: &proto.Fixed{
	//		HeaderFlag: proto.UNSUBACK,
	//		MsgLen:     uint32(2 + 1 + len(sp.TopicFilterList)), // 标识符 2 字节 + 属性1 + 业务
	//		Data:       nil,
	//	},
	//	// 和订阅协议相同
	//	PacketIdentifier: sp.PacketIdentifier,
...
func (s *PUBLISHRouter) Handle(request *Request) {
	...
	*/
	...
	case proto.QoS1:
		...
		// 返回 puback
		p := proto.NewPUBACKProtocol(sp.PacketIdentifier, proto.Success)
	...
	case proto.QoS2:
		...
		*/
		...
		// 默认成功
		p := proto.NewPUBRECProtocol(sp.PacketIdentifier, proto.Success)
...
func (s *PUBRELRouter) Handle(request *Request) {
	...
	// 需要返回PUBCOMP
	p := proto.NewPUBCOMPProtocol(sp.PacketIdentifier, proto.Success)
...
业务方可以解析 标识符合发布的标识符对应
*/
func (s *AUTHRouter) Handle(request *Request) {
	sp := request.GetProto().(*proto.AUTHProtocol)
	fmt.Printf("AUTHRouter  == %#v\n", sp)
